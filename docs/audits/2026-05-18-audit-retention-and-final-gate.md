...
- 当前版本不等同于正式合规档案系统全面上线。正式上线前仍需要真实附件上传/迁移、正式审计归档流程、定期恢复演练记录、公网专项验收和业务方合规验收。
...
- 试点阶段审计日志至少 180 天保留；若 NAS 磁盘容量不足，应先导出 PostgreSQL 备份并记录 deploy revision，再由负责人确认归档或清理窗口。
...
- 审计日志目前是查询型台账，不提供前端删除、导出或自动归档功能；正式合规档案系统上线前必须补充导出/归档/保全策略。
...
- `Storage Key` 只允许在系统设置的附件管理中由有权限账号维护；外部项目点账号不能直接创建任意附件 metadata。
