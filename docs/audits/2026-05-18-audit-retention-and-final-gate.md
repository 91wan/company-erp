...
- 试点期建议每周导出一次 CSV，并在每次导出文件名或旁路记录中写明 deploy revision、导出人、导出时间和筛选条件；导出后记录响应头 SHA256 和 record count，并用 `npm run audit:verify-export -- --csv <outside-git-path>/audit.csv --sha256 <header-sha256> --record-count <header-count>` 复核保留文件。保存位置应与 PostgreSQL dump、attachments archive、NAS `.env` 备份一起纳入受控备份边界。
