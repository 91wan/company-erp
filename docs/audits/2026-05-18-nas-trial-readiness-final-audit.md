...
- 当前版本不等同于正式合规档案系统全面上线：真实附件上传、附件迁移、OCR、长期审计归档、正式公网方案和更细颗粒度合规明细仍是后续工作。
...
- 外部项目点门户: `external_project_site` 使用任务优先视图，显示绑定项目点、项目经理信息、合规状态、红色任务、待审核/驳回资料和领用状态；明细不足处明确显示后续开放。
...
- 正式合规档案系统前，应补真实附件上传/下载闭环、附件 owner 迁移、备份恢复演练、审计归档策略和公网安全专项验收。
