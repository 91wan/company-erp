...
attempt=0
until "$DOCKER_BIN" exec "$container_name" pg_isready -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" >/dev/null 2>&1; do
  attempt=$((attempt + 1))
...
"$DOCKER_BIN" exec -i "$container_name" psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 <<'SQL'
...
"$DOCKER_BIN" exec "$container_name" pg_dump -h "$POSTGRES_HOST" -U "$POSTGRES_USER" "$POSTGRES_DB" > "$dump_file"
if [ ! -s "$dump_file" ]; then
...
"$DOCKER_BIN" exec -i "$container_name" psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 < "$dump_file"

result="$("$DOCKER_BIN" exec "$container_name" psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -At -v ON_ERROR_STOP=1 -c "SELECT count(*) || ':' || min(sample_name) || ':' || sum(quantity) FROM backup_restore_smoke;")"
