...
function usage() {
  console.log(`Usage: npm run ops -- evidence-template -- --output <outside-git-path>
...
export function createTemplates(outputDir) {
  ...
  const files = {
    ...
    "pilot-ready.README.md": [
      "# pilot-ready.txt",
      "",
      "Run `npm run ops -- trial-ready` and save the actual command output as `pilot-ready.txt`.",
    ...
    "production-ready.README.md": [
      ...
      "",
      "Run `npm run ops -- internal-ready` and save the actual command output as `production-ready.txt`.",
    ...
    "import-pilot-check.README.md": [
      ...
      "",
      "Run `npm run ops -- import-pilot-check` and save the actual command output as `import-pilot-check.txt`.",
    ...
    "import-pilot-smoke.README.md": [
      ...
      "",
      "Run `npm run ops -- import-pilot-smoke` and save the actual command output as `import-pilot-smoke.txt`.",
    ...
    "attachment-legacy-report.README.md": [
      ...
      "",
      "Run `npm run ops -- attachments-legacy-report -- --json --output <outside-git-path>/attachment-legacy-report.json`.",
    ...
    "attachment-production-check.README.md": [
      ...
      "",
      "Run `npm run ops -- attachments-production-check -- --legacy-report <outside-git-path>/attachment-legacy-report.json`.",
    ...
    "audit-export-verify.README.md": [
      ...
      "",
      "Run `npm run ops -- audit-verify-export -- --csv <outside-git-path>/audit-export.csv --sha256 <sha256> --record-count <count>`.",
    ...
    "access-review-check.README.md": [
      ...
      "",
      "Run `npm run ops -- access-review-check -- --export <outside-git-path>/access-review-export.json`.",
    ...
    "health-check.README.md": [
      "# health-check.txt",
      "",
      "Run `npm run ops -- health-check -- --base-url http://<nas>:8080` or capture `/health 200` proof.",
    ...
    "production-migration-plan-check.README.md": [
      ...
      "Run: npm run ops -- migration-plan-check -- --plan <outside-git-path>/production-migration-plan.md",
    ...
    "production-cutover-check.README.md": [
      ...
      "",
      "Copy `production-cutover-checklist.template.md` to `production-cutover-checklist.md`, replace placeholders, run `npm run ops -- cutover-check -- --checklist <outside-git-path>/production-cutover-checklist.md`, and save the actual output as `production-cutover-check.txt`.",
      "",
      ...
      "Run `npm run ops -- internal-go-live-check` after saving to verify cross-validation passes.",
      "",
    ].join("\n"),
    ...
    "post-go-live-24h/README.md": [
      ...
      "Run `npm run ops -- post-go-live-24h -- --evidence-dir <outside-git-path>/post-go-live-24h` and save the output after the 24h review.",
      "",
    ].join("\n"),
    "evidence-sha256-manifest.README.md": [
      "# evidence-sha256-manifest.json / evidence-sha256-manifest.txt",
      "",
      "Run `npm run ops -- evidence-seal -- --evidence-dir <outside-git-path>` after all P0 evidence files are in place.",
      "This generates the SHA-256 hash manifest for tamper detection.",
      "",
      "After sealing, do not modify any evidence files — re-run production:evidence-seal after any change.",
      "The final go-live approval command uses `--require-seal` to enforce seal verification.",
      "",
    ].join("\n"),
    "data-quality-report.README.md": [
      "# data-quality-report.json / data-quality-check.txt",
      "",
      "Run: DATABASE_URL=postgresql://... npm run ops -- data-quality-check -- --json --output <outside-git-path>/data-quality-report.json",
      "Save PRODUCTION_DATA_QUALITY_PASS to data-quality-check.txt.",
      "",
      "Output must be stored outside the Git repository.",
      "Output does not include phone numbers, storageKey values, or password hashes.",
      "",
    ].join("\n"),
    "data-quality-check.README.md": [
      "# data-quality-check.txt",
      "",
      "Save the text output of `npm run ops -- data-quality-check` as data-quality-check.txt.",
      "The file must contain PRODUCTION_DATA_QUALITY_PASS.",
      "",
    ].join("\n"),
    "business-acceptance.template.md": [
      "# Business Acceptance",
      "",
      "- 业务负责人:",
      "- 验收日期:",
    ...
    "business-acceptance-check.README.md": [
      "# business-acceptance-check.txt",
      "",
      "Copy business-acceptance.template.md to business-acceptance.md, fill in all fields.",
      "Run: npm run ops -- business-acceptance-check -- --acceptance <outside-git-path>/business-acceptance.md",
      "Save the output as business-acceptance-check.txt.",
      "",
      "The business owner (业务负责人) must sign the document.",
      "P0 未解决问题数量 must be 0.",
      "The document must contain 批准进入公司内网正式上线.",
      "",
    ].join("\n"),
