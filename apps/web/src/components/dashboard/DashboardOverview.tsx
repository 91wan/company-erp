...
export function DashboardOverview({
  ...
  const isProjectSiteScoped =
    currentUser.roles.includes("project_site") ||
    currentUser.roles.includes("external_project_site");
...
function purchaseRecordStatusLabel(
  status: PurchaseRecordDto["status"],
): string {
...
function projectUsageStatusLabel(
  status: ProjectUsageRequestDto["status"],
): string {
...
function certificateStatusLabel(
  status: CertificateRecordDto["computedStatus"],
): string {
...
function buildMetrics(data: DashboardLiveData): MetricCardType[] {
  ...
  const pendingApprovalCount = data.purchaseRequests.data.filter(
    (request) => request.status === "pending_approval",
  ).length;
  const pendingUsageCount = data.projectUsageRequests.data.filter(
    (request) => request.status === "pending",
  ).length;
  const expiredCertificateCount = data.certificates.data.filter(
    (certificate) => certificate.computedStatus === "expired",
  ).length;
  const expiredContractCount = data.contracts.data.filter(
    (contract) => contract.expiryState === "expired",
  ).length;
  const blockingComplianceSiteCount =
    data.projectSiteComplianceSummaries.data.filter(
      (summary) => summary.blockingIssueCount > 0,
    ).length;
  const warningCertificateCount = data.certificates.data.filter((certificate) =>
    ["expiring_soon", "review_due", "review_due_soon"].includes(
      certificate.computedStatus,
    ),
  ).length;
  const warningContractCount = data.contracts.data.filter(
    (contract) => contract.expiryState === "expiring_soon",
  ).length;
  const pendingCertificateReviewCount = data.certificates.data.filter(
    (certificate) =>
      certificate.isComplianceCritical &&
      !certificate.confirmedAt &&
      !certificate.isDisabled,
  ).length;
  const lowStockCount = data.inventoryBalances.data.filter(
    (balance) => balance.isLowStock,
  ).length;
  const todoCount = pendingApprovalCount + pendingUsageCount;
  const dangerCount =
    expiredCertificateCount +
    expiredContractCount +
    blockingComplianceSiteCount;
  ...
  return [
    {
      ...
      detail:
        ...
        data.projectUsageRequests.status === "error"
          ? "数据暂不可用"
          : "采购审批与领用处理",
      tone: "blue",
      icon: ClipboardCheck,
    },
    {
      ...
      detail:
        ...
        data.projectSiteComplianceSummaries.status === "error"
          ? "数据暂不可用"
          : "已过期合同/证照与项目点合规",
      tone: "orange",
      icon: PackageCheck,
    ...
    },
    {
      ...
      detail:
        data.inventoryBalances.status === "error"
          ? "数据暂不可用"
          : "低于安全库存",
      tone: "orange",
      icon: PackageCheck,
...
function summaryItemToQueueItem(
    ...
    : sortByUpdatedAt(data.certificates.data)
        ...
        .map((certificate) => ({
          ...
          owner:
            ...
            certificate.ownerProjectSiteName ||
            certificate.ownerPartyName ||
            "-",
    ...
    >
      ...
      <DataTable
        ...
        emptyState={
          <EmptyState
            title="暂无待办"
            description="当前没有采购审批、领用处理或证照复核事项。"
          />
        }
...
function RiskQueuePanel({
  ...
  const contractRisks: QueueItem[] = summary
    ...
    : sortByUpdatedAt(data.contracts.data)
        .filter(
          (contract) =>
            contract.expiryState === "expired" ||
            contract.expiryState === "expiring_soon",
        )
        .slice(0, 3)
        .map((contract) => ({
  ...
  const projectSiteComplianceRisks: QueueItem[] = summary
    ...
    : data.projectSiteComplianceSummaries.data
        .filter(
          (summary) =>
            summary.blockingIssueCount > 0 || summary.warningIssueCount > 0,
        )
        .slice(0, 3)
        .map((summary) => ({
  ...
  return (
    <SectionCard
      ...
      badge={
        <UiStatusBadge
          tone={
            rows.some((row) => row.tone === "danger") ? "danger" : "warning"
          }
        >
          {rows.length} 项
        </UiStatusBadge>
...
function LowStockPanel({
  ...
  if (dashboardSummary) {
    ...
    return (
      <section className="dashboard-panel table-panel">
        ...
        {!dashboardSummary.unavailableSections.includes("inventory") &&
        rows.length === 0 ? (
          <PanelStateMessage text="暂无低库存物料" />
        ) : null}
        <ResponsiveTable
          ...
          rows={rows.map((row) => [
            ...
            <UiStatusBadge
              key={`${row.id}-status`}
              tone={inventoryRiskToBadge(true).tone}
            >
              {row.statusLabel ?? "低库存"}
            </UiStatusBadge>,
          ])}
  ...
  const rows = inventoryBalances.data
    .filter((balance) => balance.isLowStock)
    .slice(0, 5);
  return (
    <section className="dashboard-panel table-panel">
      ...
      {inventoryBalances.status !== "error" && rows.length === 0 ? (
        <PanelStateMessage text="暂无低库存物料" />
      ) : null}
      <ResponsiveTable
        headers={["物料编码", "物料名称", "当前库存", "安全库存", "状态"]}
        ...
        rows={rows.map((row) => [
          ...
          <UiStatusBadge
            key={`${row.materialCode}-status`}
            tone={inventoryRiskToBadge(row.isLowStock).tone}
          >
...
function SystemStatusPanel({
  ...
  const items = [
    ...
    },
    {
      label: "应用版本",
      detail:
        appVersion.status === "success" && appVersion.data
          ? appVersion.data.shortCommitSha
          : "版本信息不可用",
      side:
        appVersion.status === "success" && appVersion.data
          ? appVersion.data.environment
          : "检查系统设置",
