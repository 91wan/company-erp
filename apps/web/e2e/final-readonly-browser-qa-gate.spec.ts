...
test("admin final readonly gate uses dashboard summary and keeps audit, attachments, and project-site controls usable", async ({ page }) => {
  ...
  expect(requestedUrls.some((url) => new URL(url).pathname === "/api/dashboard/summary")).toBe(true);
  expect(requestedUrls.some((url) => new URL(url).pathname.includes("/compliance-summary"))).toBe(false);
...
test("viewer final readonly gate hides business mutation, audit, and attachment management actions", async ({ page }) => {
  const issues = trackBrowserIssues(page);
...
test("project_site final readonly gate keeps scoped usage without global inventory or sensitive amount surfaces", async ({ page }) => {
  const issues = trackBrowserIssues(page);
...
test("external_project_site final readonly gate stays portal-only across compliance sections", async ({ page }) => {
  const issues = trackBrowserIssues(page);
