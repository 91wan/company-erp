...
test("admin final ProjectSites QA keeps risk ledger, detail drawer, attachments, issue confirmation, and equipment usable", async ({ page }) => {
  ...
  await page.getByLabel("出库单号").fill("DEMO-QA-FINAL-OUT");
...
test("viewer final ProjectSites QA keeps the workspace read-only", async ({ page }) => {
  const issues = trackBrowserIssues(page);
...
test("project_site final ProjectSites QA keeps scoped usage available and sensitive amount surfaces hidden", async ({ page }) => {
  const issues = trackBrowserIssues(page);
