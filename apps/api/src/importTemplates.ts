...
export const IMPORT_TEMPLATE_DEFINITIONS: Record<ImportTemplateTypeCode, TemplateDefinition> = {
  ...
  materials: {
    ...
    sample: ["MAT0001", "示例物料", "500g", "食材", "件", "SUP0001", 10, "启用", ""],
    notes: ["基本单位用于库存和领用数量。", "确认导入时编码重复的物料会跳过，不会覆盖。"],
  },
  employees: {
  ...
  opening_inventory: {
    ...
    sample: ["WH0001", "MAT0001", 10, "件", "2026-05-01", "", "总部仓库", ""],
  ...
  health_certificates: {
    headers: ["项目点编码", "项目点现场人员姓名", "身份证后四位", "健康证编号", "到期日期", "签发日期", "发证机构", "图片文件名", "备注"],
    sample: ["SITE0001", "王示例", "1234", "HC20260501001", "2027-05-01", "2026-05-01", "示例机构", "王示例健康证.png", ""],
    notes: ["图片文件名可为空；缺图片不阻断到期提醒导入。", "健康证必须匹配 active 项目点现场人员。"],
  },
};
